...
class Server {
	constructor( name = 'Default', port = 2000 ) {
		...
		(async () => {
			...
			console.log( `\nServer is ready and waiting for incoming connections...\n-----` );
	...
	async banIP( addr, duration ) {
		const subproc = childProc.spawn( 'iptables', [ '-I', 'INPUT', '-s', addr, '-j', 'DROP' ], { detached: true, stdio: 'ignore' } );
		subproc.unref();
	...
	async unbanIP( addr ) {
		const subproc = childProc.spawn( 'iptables', [ '-D', 'INPUT', '-s', addr, '-j', 'DROP' ], { detached: true, stdio: 'ignore' } );
		subproc.unref();
